...
unsigned int devman_get_devices_by_con_type(enum dev_con_types con_type, unsigned int** result); // Получение устройств по типу. Возвращает количество, в result указатель на массив индексов устройств
...
struct dev_info* devman_get_first_device_by_specs(enum dev_con_types con_type, uint8_t classcode, uint8_t subclass); // Возвращает указатель на первое устройство, подходящее под параметры, или 0, если не найдено
